...
export const speedToPace = (speed: number) => {
    ...
    return { minutes: Math.floor(60 / speed), seconds: Math.floor((60 / speed - Math.floor(60 / speed)) * 60) }
}
...
export const timeBase10ToMinSec = (time: number) => {
    return { hours: Math.floor(time / 60), minutes: Math.floor(time % 60), seconds: Math.floor((time - Math.floor(time)) * 60)}
}
